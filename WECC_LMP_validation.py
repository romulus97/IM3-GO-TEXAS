...
def LMP_validation(sim_LMP_name):
    ...
    return R2_weighted_average_demand
...
for NN in node_numbers:
    ...
    for UC in UC_treatment:
        ...
        for TP in transmission_multiplier:
            ...
            sim_LMP_name = 'Model_results/duals_{}_{}_{}.csv'.format(NN,UC,TP)
            R2_vals = LMP_validation(sim_LMP_name) 
            all_rows = [sim_LMP_name[20:-4]] + R2_vals
            all_R2_data.append(all_rows)
            
all_R2_df = pd.DataFrame(all_R2_data, columns=['Type', 'MidC', 'PaloVerde', 'PGAE', 'SCE', 'SDGE'])
all_R2_df.to_excel('LMP_R2_values.xlsx',index=False)           
