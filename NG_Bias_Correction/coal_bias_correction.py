...
for year in years:
    ...
    NG_estimated = pd.read_csv('NG_data/Estimated_NG_prices/Average_NG_prices_BAs_{}.csv'.format(year))
    del NG_estimated['Unnamed: 0']
    NG_estimated.index = time_range
    
    #Reading monthly BA NG data
    BA_NG_df = pd.read_excel('Monthly_fuel_prices/{}_Fuel_Price_Data.xlsx'.format(year),sheet_name='State_coal')
    ...
    NG_estimated.to_csv('NG_data/Bias_corrected_NG_prices/Average_NG_prices_BAs_{}.csv'.format(year),index=False)
