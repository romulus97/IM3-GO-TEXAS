...
df_full = pd.read_csv('M2S_nodes.csv',header=0)
df_selected = pd.read_csv('M2S_nodes.csv',header=0)
df_branches = pd.read_csv('M2S_ind_lines.csv',header=0)
...
for b in buses:
    if buses.index(b) < 1:
        df_selected_GPS = df_full.loc[df_full['bus_i']==b,:]
    else:
        a = df_full.loc[df_full['bus_i']==b,:]
...
pos = {}
for count, elem in enumerate (df_selected_GPS['bus_i']):
    pos[elem] = (mx[count], my[count])

nx.draw_networkx_nodes(G = graph, pos = pos, node_list = graph.nodes(), 
                        node_color = 'red', alpha = 0.8, node_size = 8)
