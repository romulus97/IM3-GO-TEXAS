...
hydro_name_id = pd.read_csv('EIA_ERCOT_hydro_plants_to_generators.csv',header=0\
                            ,usecols=['EIA_ID','plant','state','bal_auth','bus'])
    
hydro_cap = pd.read_csv('targets_WECC_ERCOT_hydro_epiweek_mean_MW.csv',header=0)
...
for ind,row in hydro_name_id.iterrows():
    ...
    if selected_plant.empty:
        ...
        capacity = selected_plant['nameplate'].values[0]
        plant_caps.append(capacity)
...
hydro_name_id_updated.to_csv('EIA_ERCOT_reduced_hydro_plants.csv', index=False)
...
weekly_gen_final.to_csv('p_mean_max_min_MW_ERCOTplants_weekly_2019.csv',index=False)
